    ...
    switch (id) {
      ...
      case 2: {
        ...
        for (int i = 0; i < imports.size(); i++) {
          ...
          switch (imports[i].desc.tag) {
            case importdesc::FUNC:
              std::cout << "FUNC :: " << types[imports[i].desc.func]
                        << std::endl;
              break;
            case importdesc::TABLE:
              std::cout << "TABLE :: from " << imports[i].desc.table.limits.min
                        << " to " << imports[i].desc.table.limits.max
                        << std::endl;
              break;
            case importdesc::MEM:
              std::cout << "MEM :: from " << imports[i].desc.mem.limits.min
      ...
      case 4: {
        ...
        for (unsigned int i = 0; i < tables.size(); i++) {
          std::cout << "TABLE :: from " << tables[i].type.limits.min << " to "
                    << tables[i].type.limits.max << std::endl;
        }
        std::cout << "----- ------ -----\n";
#endif
        break;
      ...
      case 5: {
        ...
        for (unsigned int i = 0; i < mems.size(); i++) {
          std::cout << "MEM :: from " << mems[i].type.limits.min << " to "
                    << mems[i].type.limits.max << std::endl;
        }
        std::cout << "----- -------- -----\n";
#endif
        break;
      ...
      case 6: {
        ...
        for (unsigned int i = 0; i < globals.size(); i++) {
          std::cout << "GLOBAL ::  " << globals[i].type.value
                    << ((globals[i].type.mut) ? " (mut)" : "") << " init to "
                    << globals[i].init << std::endl;
        }
        std::cout << "----- ------- -----\n";
#endif
        break;
      ...
      case 7: {
        ...
        for (unsigned int i = 0; i < exports.size(); i++) {
          ...
          switch (exports[i].desc.tag) {
            case importdesc::FUNC:
              std::cout << "FUNC :: " << types[funcs[exports[i].desc.func].type]
                        << std::endl;
              break;
            case importdesc::TABLE:
              std::cout << "TABLE :: from "
                        << tables[exports[i].desc.table].type.limits.min
                        << " to "
                        << tables[exports[i].desc.table].type.limits.max
                        << std::endl;
              break;
            case importdesc::MEM:
              std::cout << "MEM :: from "
                        << mems[exports[i].desc.mem].type.limits.min << " to "
                        << mems[exports[i].desc.mem].type.limits.max
                        << std::endl;
              break;
            case importdesc::GLOBAL:
              // TODO: uncomment when globals are parsed (seg fault here)
              std::cout << "GLOBAL :: "
                        << globals[exports[i].desc.global].type.value
                        << ((globals[exports[i].desc.global].type.mut) ? "(mut)"
                                                                       : "")
                        << globals[exports[i].desc.global].init << std::endl;
              break;
          }
        }
        std::cout << "----- ------- -----\n";
      ...
      case 8: {
        ...
        ASSERT((startidx < funcs.size()),
               "Start is a funcidx so it has to be < funcs.size()\n");
        warn("Parsing Start complete\n");
#if DEBUG
        std::cout << "START func is " << startidx
                  << " :: " << types[funcs[startidx].type] << std::endl;
#endif
        break;
      }
      case 9: {
        ...
        for (unsigned int i = 0; i < elems.size(); i++) {
          std::cout << "ELEM :: table " << elems[i].table << " at "
                    << elems[i].offset << " has funcs:" << std::endl;
          for (unsigned int j = 0; j < elems[i].init.size(); j++) {
            std::cout << "\t" << elems[i].init[j]
                      << " :: " << types[funcs[elems[i].init[j]].type]
                      << std::endl;
          }
        }
        std::cout << "----- ------- -----\n";
      ...
      case 11: {
        ...
        for (unsigned int i = 0; i < datas.size(); i++) {
          ...
          std::cout << "DATA :: mem " << datas[i].data << " at "
                    << datas[i].offset << " has bytes:" << content << std::endl;
        }
        std::cout << "----- ----- -----\n";
#endif
        break;
